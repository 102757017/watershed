...
# 生成画布
fig=plt.figure()
img = cv2.imdecode(np.fromfile('7.png', dtype=np.uint8), 1)
...
foreground=cv2.distanceTransform(binary, cv2.DIST_L1,3)
...
#分离前景区域
ret,foreground=cv2.threshold(foreground,4,255,cv2.THRESH_BINARY)
#转换数据类型
